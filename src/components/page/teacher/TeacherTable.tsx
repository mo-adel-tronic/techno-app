...
import { deleteBulkTeachers, deleteTeacher } from "@/features/teachers/TeacherRepo"
...
export default function TeacherTable({data} : Props) {
    ...
    const [teacherTable, setTeacherTable] = useState<TableType<Teacher> | null>(null)
    const router = useRouter();
    const { revalidate } = useRevalidate();
    const confirmBulkDelete = async () => {
        if (rowToDelete) {
          // Single row deletion
          var res = await deleteTeacher(rowToDelete.id || 0)
          //! delete single item
          setRowToDelete(null)
        } else {
          // Bulk deletion
          const selectedRows = teacherTable ? teacherTable.getFilteredSelectedRowModel().rows : []
          const selectedIds = selectedRows.map(row => row.original.id || 0);
          var res = await deleteBulkTeachers(selectedIds);
            //! Delete Bulk items
  ...
  return (
    <div>
        {/* Table */}
        <AppTable<Teacher> data={data} columns={columns} BulkActions={handleBulkDelete} />
